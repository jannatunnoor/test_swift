...
class TempAuth(object):
    ...
    def __call__(self, env, start_response):
        ...
        """
        print('++++++++++++++++++start (__call__)+++++++++++++++++++ in /middleware/tempauth.py')
        print('env = %s and start_response = %s ' % (env, start_response))
        if self.allow_overrides and env.get('swift.authorize_override', False):
            print('self.allow_overrides and env.get(swift.authorize_override)')
            return self.app(env, start_response)
        if env.get('PATH_INFO', '').startswith(self.auth_prefix):
            print('env.get(PATH_INFO, '').startswith(self.auth_prefix)')
            return self.handle(env, start_response)
        s3 = env.get('HTTP_AUTHORIZATION')
        print('s3 = %s ' % s3)
        token = env.get('HTTP_X_AUTH_TOKEN', env.get('HTTP_X_STORAGE_TOKEN'))
        service_token = env.get('HTTP_X_SERVICE_TOKEN')
        print('token = %s and service_token = %s ' % (token,service_token))
        if s3 or (token and token.startswith(self.reseller_prefix)):
                ...
                if self.reseller_prefix and not s3:
                    ...
                    try:
                        vrs, realm, rest = split_path(env['PATH_INFO'],
                                                      2, 3, True)
                        print('vrs = %s , realm = %s and rest = %s'%(vrs,realm,rest))
                    except ValueError:
                        realm = 'unknown'
            ...
            if self._is_definitive_auth(env.get('PATH_INFO', '')):
                ...
                if 'swift.authorize' not in env:
                    env['swift.authorize'] = self.denied_response
        print('env in tempauth = %s '% env )
        print('++++++++++++++++++end (__call__)+++++++++++++++++++ in /middleware/tempauth.py')
        return self.app(env, start_response)
    ...
    def _is_definitive_auth(self, path):
        ...
        """
        print('++++++++++++++++++start (_is_definitive_auth)+++++++++++++++++++ in /middleware/tempauth.py')
        print('path = %s ' % path)
        try:
            version, account, rest = split_path(path, 1, 3, True)
        except ValueError:
            print('++++++++++++++++++end (_is_definitive_auth)+++++++++++++++++++ in /middleware/tempauth.py')
            return False
        if account:
            print('++++++++++++++++++end (_is_definitive_auth)+++++++++++++++++++ in /middleware/tempauth.py')
    ...
    def _non_empty_reseller_prefixes(self):
        print('++++++++++++++++++start (_non_empty_reseller_prefixes) and return +++++++++++++++++++ in /middleware/tempauth.py')
    ...
    def _get_account_prefix(self, account):
        ...
        """
        print('++++++++++++++++++start (_get_account_prefix)+++++++++++++++++++ in /middleware/tempauth.py')
        ...
        for prefix in self._non_empty_reseller_prefixes():
            if account.startswith(prefix):
                print('++++++++++++++++++end (_get_account_prefix)+++++++++++++++++++ in /middleware/tempauth.py')
                return prefix
    ...
    def _dot_account(self, account):
        ...
        """
        print('++++++++++++++++++start (_dot_account)+++++++++++++++++++ in /middleware/tempauth.py')
        print('account = %s ' % account)
        prefix = self._get_account_prefix(account)
        print('++++++++++++++++++end (_dot_account)+++++++++++++++++++ in /middleware/tempauth.py')
    ...
    def _get_user_groups(self, account, account_user, account_id):
        ...
        """
        print('++++++++++++++++++start (_get_user_groups)+++++++++++++++++++ in /middleware/tempauth.py')
        print('account = %s , account_usr =%s and account_id = %s '%(account,account_user,account_id))
        ...
        groups = ','.join(groups)
        print('groups = %s '% groups)
        print('++++++++++++++++++end (_get_user_groups)+++++++++++++++++++ in /middleware/tempauth.py')
        return groups
    ...
    def get_groups(self, env, token):
        ...
        """
        print('+++++++++++++++++++++ start (get_groups)+++++++++++++++++++ in /middleware/tempauth.py')
        pprint('env = %s and token = %s '%(env,token))
        groups = None
        ...
        print('memcache_token_key = %s and cached_auth_data = %s '%(memcache_token_key,cached_auth_data))
        if cached_auth_data:
        ...
        print('groups = %s '%groups)
        print('++++++++++++++++++end (get_groups)+++++++++++++++++++ in /middleware/tempauth.py')
        return groups
    ...
    def extract_acl_and_report_errors(self, req):
        ...
        """
        print('++++++++++++++++++start (extract_acl_and_report_errors)+++++++++++++++++++ in /middleware/tempauth.py')
        acl_header = 'x-account-access-control'
        acl_data = req.headers.get(acl_header)
        result = parse_acl(version=2, data=acl_data)
        print('acl_header = %s , acl_data = %s and result = %s '%(acl_header,acl_data,result))
        if result is None:
            print('++++++++++++++++++end (extract_acl_and_report_errors)+++++++++++++++++++ in /middleware/tempauth.py')
        ...
        print('internal_hdr = %s and request_headers = %s '%req.headers)
        print('++++++++++++++++++end (extract_acl_and_report_errors)+++++++++++++++++++ in /middleware/tempauth.py')
        return None
    ...
    def authorize(self, req):
        ...
        """
        print('++++++++++++++++++start (authorize)+++++++++++++++++++ in /middleware/tempauth.py')
        print('req = %s '%req)
        try:
            _junk, account, container, obj = req.split_path(1, 4, True)
            print('_junk = %s, account = %s, container = %s , obj = %s '%(_junk, account, container, obj))
        except ValueError:
            self.logger.increment('errors')
            print('++++++++++++++++++end (authorize)+++++++++++++++++++ in /middleware/tempauth.py')
            return HTTPNotFound(request=req)

        if self._get_account_prefix(account) is None:
            print('self._get_account_prefix(account) is None')
            self.logger.debug("Account name: %s doesn't start with "
                              "reseller_prefix(s): %s."
                              % (account, ','.join(self.reseller_prefixes)))
            print('++++++++++++++++++end (authorize)++ by denied response self._get_account_prefix(account) is None+++++++++++++++++ in /middleware/tempauth.py')
        ...
        #*********************************
        print('req.remote_user = %s '%req.remote_user)
        #*********************************
        user_groups = (req.remote_user or '').split(',')
        account_user = user_groups[1] if len(user_groups) > 1 else None
        print('user_groups = %s and account_user = %s '%(user_groups,account_user))
        if '.reseller_admin' in user_groups and \
                account not in self.reseller_prefixes and \
                not self._dot_account(account):
            print('.reseller_admin in user_groups and account not in self.reseller_prefixes and not self._dot_account(account):')
        ...
        if account in user_groups and \
            ...
            print('account in user_groups and (req.method not in (DELETE, PUT) or container):')
            account_prefix = self._get_account_prefix(account)
            require_group = self.account_rules.get(account_prefix).get(
                'require_group')
            print('account_prefix = %s and require_group = %s '%(account_prefix,require_group))
            if require_group and require_group in user_groups:
                print('if require_group and require_group in user_groups:')
                req.environ['swift_owner'] = True
        ...
        if (req.environ.get('swift_sync_key')
            ...
            print('Allow request with container sync-key:')
        ...
        if req.method == 'OPTIONS':
            ...
            print('req.method == OPTIONS')
            self.logger.debug("Allow OPTIONS request.")
            print('++++++++++++++++++end (authorize)+++++++++++++++++++ in /middleware/tempauth.py')
            return None

        referrers, groups = parse_acl(getattr(req, 'acl', None))
        print('referrers = %s and groups = %s '%(referrers,groups))
        if referrer_allowed(req.referer, referrers):
            print('referrer_allowed(req.referer, referrers):')
        ...
        if acct_acls:
            ...
            print('user_group_set = %s ' % user_group_set)
            ...
            if (user_group_set.intersection(acct_acls['read-write']) and
                ...
                print('user_group_set.intersection(acct_acls[read-write]) and (container or req.method in (GET, HEAD))')
            ...
            if (user_group_set.intersection(acct_acls['read-only']) and
                ...
                print('user_group_set.intersection(acct_acls[read-only]) and (container or req.method in (GET, HEAD))')
                self.logger.debug('User %s allowed by X-Account-Access-Control'
                                  ' (read-only)' % account_user)
                print('++++++++++++++++++end (authorize)+++++++++++++++++++ in /middleware/tempauth.py')
                return None
            print('no condition is held :( ')
        print('++++++++++++++++++end (authorize)+++++++++++++++++++ in /middleware/tempauth.py')
        return self.denied_response(req)
    ...
    def denied_response(self, req):
        ...
        """
        print('++++++++++++++++++start (denied_response)+++++++++++++++++++ in /middleware/tempauth.py')
        print('req = %s '%req)
        if req.remote_user:
            self.logger.increment('forbidden')
            print('++++++++++++++++++end (denied_response)+++++++++++++++++++ in /middleware/tempauth.py')
    ...
    def handle(self, env, start_response):
        ...
        """
        print('++++++++++++++++++start ( handle )+++++++++++++++++++ in /middleware/tempauth.py')
        pprint('env = %s and start_response = %s '%(env,start_response))
        try:
            req = Request(env)
            print('req = %s '%req)
            if self.auth_prefix:
                print('self.auth_prefix')
                req.path_info_pop()
            req.bytes_transferred = '-'
            req.client_disconnect = False
            print('req.headers = %s '%req.headers)

            if 'x-storage-token' in req.headers and \
                    'x-auth-token' not in req.headers:
                req.headers['x-auth-token'] = req.headers['x-storage-token']

            print('++++++++++++++++++end ( handle )+++++++++++++++++++ in /middleware/tempauth.py')
    ...
    def handle_request(self, req):
        ...
        """
        print('++++++++++++++++++start ( handle_request )+++++++++++++++++++ in /middleware/tempauth.py')
        print('req = %s ' % req)
        req.start_time = time()
        handler = None
        try:
            version, account, user, _junk = req.split_path(1, 4, True)
            print('version = %s , account = %s , user = %s , _junk = %s '%(version,account,user,_junk))
        except ValueError:
            self.logger.increment('errors')
            print('++++++++++++++++++end ( handle_request )+++++++++++++++++++ in /middleware/tempauth.py')
            return HTTPNotFound(request=req)
        ...
        print('request_response = %s '%req.response)
        print('++++++++++++++++++end ( handle_request )+++++++++++++++++++ in /middleware/tempauth.py')
        return req.response
    ...
    def handle_get_token(self, req):
        ...
        print('++++++++++++++++++start ( handle_get_token )+++++++++++++++++++ in /middleware/tempauth.py')
        print('req = %s ' % req)
        try:
            pathsegs = split_path(req.path_info, 1, 3, True)
            print('pathsegs = %s '%pathsegs)
        except ValueError:
            self.logger.increment('errors')
        ...
        if pathsegs[0] == 'v1' and pathsegs[2] == 'auth':
            ...
            if not user:
                ...
                print('account2 = %s , user = %s '%(account2,user))
                if account != account2:
                    self.logger.increment('token_denied')
                    print('++++++++++++++++++end ( handle_get_token )+++++++++++++++++++ in /middleware/tempauth.py')
                    return HTTPUnauthorized(request=req, headers=
            ...
            user = req.headers.get('x-auth-user')
            print('elif pathsegs[0] in (auth, v1.0):  user = %s'%user)
            if not user:
                user = req.headers.get('x-storage-user')
            if not user or ':' not in user:
                self.logger.increment('token_denied')
            ...
            print('account = %s , user = %s '%(account,user))
            key = req.headers.get('x-auth-key')
            print('key = %s'%key)
            if not key:
                key = req.headers.get('x-storage-pass')
            ...
            print('++++++++++++++++++end ( handle_get_token )+++htttp bad request ++++++++++++++++ in /middleware/tempauth.py')
...
def filter_factory(global_conf, **local_conf):
    ...
    print('+++++++++++++++++++start(filter_factory)+++++++++++++++++++++++in /middleware/tempauth.py')
    conf = global_conf.copy()
    conf.update(local_conf)
    register_swift_info('tempauth', account_acls=True)
    print('conf = %s '%conf)
    def auth_filter(app):
        print('+++++++++++++++start(auth_filter) and return TempAuth+++++++++++++++++++++++in /middleware/tempauth.py')
        print('app = %s '%app)
        return TempAuth(app, conf)
    print('+++++++++++++++++++end(filter_factory)+++++++++++++++++++++++in /middleware/tempauth.py')
    return auth_filter

...
def get_logger(conf, name=None, log_to_console=False, log_route=None,
    ...
    **Log config and defaults**::
    ...
    """
    print('***** def get_logger in util.py start ******')
    # print('conf = %s'%conf)
    if not conf:
        conf = {}
    if name is None:
    ...
    if not log_route:
        log_route = name
    print('conf = %s , name = %s , log_route = %s'%(conf,name,log_route))
    ...
    print('***** def get_logger in util.py end ******')
...
def capture_stdio(logger, **kwargs):
    ...
    with open(os.devnull, 'r+b') as nullfile:
		...
		print('ioerror in doing flush')
                pass

            try:
                os.dup2(nullfile.fileno(), f.fileno())
            except OSError:
		print('oserror on os.dup2')
                pass
	print('for loop in stdio_files end')

    # redirect stdio
    if kwargs.pop('capture_stdout', True):
        sys.stdout = LoggerFileObject(logger)
	print('apture_stdout true',sys.stdout)
    if kwargs.pop('capture_stderr', True):
        sys.stderr = LoggerFileObject(logger, 'STDERR')
	print('apture_stderror true',sys.stdout)
    print('capture_studio end')

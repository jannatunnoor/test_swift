...
def get_container_info(env, app, swift_source=None):
    ...
    """
    print('++++++++++++++++++start(get_container_info)+++++++++++++++++in proxy/controller/base.py')
    ...
    print('info in get_container info = %s '%info)
    print('++++++++++++++++++start(get_container_info)+++++++++++++++++in proxy/controller/base.py')
    return info
...
def get_account_info(env, app, swift_source=None):
    ...
    """
    print('++++++++++++++++++start(get_account_info)+++++++++++++++++in proxy/controller/base.py')
    ...
    print('info in get_account_info = %s '%info)
    print('++++++++++++++++++end(get_account_info)+++++++++++++++++in proxy/controller/base.py')
    return info
...
def get_info(app, env, account, container=None, ret_not_found=False,
    ...
    """
    print('++++++++++++++++++++++start(get_info)+++++++++++++++++++++++++in /controller/base.py')
    print('app = %s , account = %s and container = %s in get_info '%(app,account,container))
    print('in get_info env = %s'%env)
    info = _get_info_cache(app, env, account, container)
    print('info in get_info  = %s  '%info)	
    if info:
        if ret_not_found or is_success(info['status']):
	    print('++++++++++++++++++++++end (get_info) and return info+++++++++++++++++++++++++in /controller/base.py')
            return info
	print('++++++++++++++++++++++end(get_info)++in if info +++and return None++++++++++++++++++++in /controller/base.py')
        return None
    ...
    if container:
	    ...
	    print('++++++++++++++++++++++end(get_info)++in if not get_info +++and return None++++++++++++++++++++in /controller/base.py')
            return None
        path += '/' + container
    ...
    cache_key, env_key = _get_cache_key(account, container)
    print('req = %s , resp =%s cache_key = %s , env_key = %s'%(req,resp,cache_key,env_key))
    print('')
    try:
        info = resp.environ[env_key]
        env[env_key] = info
        if ret_not_found or is_success(info['status']):
	    print('++++++++++++++++++++++end(get_info)++final +++and return info++++++++++++++++++++in /controller/base.py')
            return info
    except (KeyError, AttributeError):
        pass
    print('++++++++++++++++++++++end(get_info)++ if exception +++and return None++++++++++++++++++++in /controller/base.py')
    return None

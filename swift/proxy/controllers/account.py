...
class AccountController(Controller):
	...
	print 'in add_acls_from_sys_metadata of accountcontroller class'
	...
	print ' in POST function of accountcontroller class'
        if len(self.account_name) > constraints.MAX_ACCOUNT_NAME_LENGTH:
            resp = HTTPBadRequest(request=req)
            resp.body = 'Account name length of %d longer than %d' % \
                        (len(self.account_name),
	...
	print 'resp',resp
        if resp.status_int == HTTP_NOT_FOUND and self.app.account_autocreate:
            ...
            resp = self.make_requests(
                ...
                req.swift_entity_path, [headers] * len(accounts))
        self.add_acls_from_sys_metadata(resp)
	print 'in POST function of accountcontroller class END'
        return resp

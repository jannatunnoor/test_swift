...
class ObjectController(BaseStorageServer):
	...
	print('in init function of objectcontroller',self.node_timeout,self.conn_timeout,self.client_timeout,self.disk_chunk_size,self.network_chunk_size,self.log_requests, self.max_upload_time,self.slow)
    ...
    def setup(self, conf):
	    ...
	    print 'self.replication_semafore',self.replication_semaphore
        else:
	...
	#print 'req_timestamp',req_timestamp
        ...
        new_delete_at = int(request.headers.get('X-Delete-At') or 0)
	print 'new_delete_at',new_delete_at
        if new_delete_at and new_delete_at < time.time():
            return HTTPBadRequest(body='X-Delete-At in past', request=request,
                                  content_type='text/plain')
        try:
	...
	#print 'request.if_none_match',request.if_none_match
        ...
        orig_timestamp = Timestamp(orig_metadata.get('X-Timestamp', 0))
	#print 'orig_timestamp,req_timestamp',orig_timestamp,req_timestamp
        if orig_timestamp >= req_timestamp:
            return HTTPConflict(
                request=request,
                headers={'X-Backend-Timestamp': orig_timestamp.internal})
        orig_delete_at = int(orig_metadata.get('X-Delete-At') or 0)
	#print 'orig_delete_at',orig_delete_at
        upload_expiration = time.time() + self.max_upload_time
	#print 'upload_expiration',upload_expiration
        etag = md5()
	#print 'etag',etag
        elapsed_time = 0
        try:
		...
		#print 'have_metadata_footer,use_multiphase_commit',have_metadata_footer,use_multiphase_commit
			...
			#print 'not mime_boundary'
			   ...
			   #print ' _junk_hdrs, obj_input',_junk_hdrs, obj_input
                    except ChunkReadTimeout:
                        return HTTPRequestTimeout(request=request)

                timeout_reader = self._make_timeout_reader(obj_input)
		#print 'timeout_reader',timeout_reader
                try:
                    for chunk in iter(timeout_reader, ''):
                        start_time = time.time()
			#print 'start_time,upload_expiration',start_time,upload_expiration
                        if start_time > upload_expiration:
                            self.logger.increment('PUT.timeouts')
                            return HTTPRequestTimeout(request=request)
                        etag.update(chunk)
                        upload_size = writer.write(chunk)
			#print 'upload_size',upload_size
			...
			#print 'elapsed_time',elapsed_time
                ...
                if upload_size:
                    self.logger.transfer_rate(
                        ...
                        upload_size)		    
                    #print  'PUT.',device,'.timing',elapsed_time,upload_size
	...
	print '**********in __call for object__***************   START '
        start_time = time.time()
        req = Request(env)
	print 'req',req
	    ...
	    print 'req.method in PUT DELETE,slow == ',slow
            if slow > 0:
                sleep(slow)
		...
		print '**********in __call for object__**************END1 else '
                return zero_copy_iter()
            else:
		print '**********in __call for object__*************** END 2 else'
                return res(env, start_response)
        else:
	    print '**********in __call for object__*************  END 3 else'
            return res(env, start_response)
...
def global_conf_callback(preloaded_app_conf, global_conf):
    ...
    """
    print '************def global_conf_callback start *********** '
    print 'preloaded_app_conf, global_conf',preloaded_app_conf,global_conf
    replication_concurrency = int(
        preloaded_app_conf.get('replication_concurrency') or 4)
    print 'replication_concurrency', replication_concurrency
    ...
    print '****def global_conf_callabck end *****'


def app_factory(global_conf, **local_conf):
    """paste.deploy app factory for creating WSGI object server apps"""
    print '*****def app_factory start *****'
    conf = global_conf.copy()
    print 'global_conf,local_conf',global_conf,local_conf
    conf.update(local_conf)
    print '****** def app_factory end*******'
